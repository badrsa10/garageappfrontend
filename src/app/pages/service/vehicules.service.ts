...
export class VehiculeService {
  private readonly API_URL = 'http://localhost:3000/api/vehicule';
  ...
  getVehicules(page: number, limit: number, search?: string): Observable<VehiculeApiResponse> {
    ...
    return this.http.get<VehiculeApiResponse>(this.API_URL, { params }).pipe(
  ...
  getVehiculeById(id: string): Observable<Vehicule> {
    return this.http.get<Vehicule>(`${this.API_URL}/${id}`).pipe(
      catchError(this.handleError)
    );
  }
  ...
  createVehicule(vehicule: Omit<Vehicule, 'id_vehicule'>): Observable<Vehicule> {
    return this.http.post<Vehicule>(this.API_URL, vehicule).pipe(
      catchError(this.handleError)
    );
  }
  ...
  updateVehicule(id: string, updates: Partial<Vehicule>): Observable<Vehicule> {
    return this.http.put<Vehicule>(`${this.API_URL}/${id}`, updates).pipe(
      catchError(this.handleError)
    );
  }
